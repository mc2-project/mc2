...
3. Ensure that SSH keys have been properly set up between the aggregator and other parties. 
    * If the machine running the aggregator doesn't yet have SSH keys set up (most likely at `~/.ssh/`), generate a 4096 bit RSA key. There should now be keys at `~/.ssh/`

        ```sh
        ssh-keygen -t rsa -b 4096
        ```

    * Otherwise, if the aggregator has already set up SSH keys, ensure that the private key is in a file named `~/.ssh/id_rsa`.
    * Set up communication between the aggregator and the worker nodes by appending the driver's public key to the `~/.ssh/authorized_keys` file of all nodes. The public key should be in `~/.ssh/id_rsa.pub`. You can manually copy and paste the public key over to each party's node.
    * Make sure that you also add the public key to the aggregator's own `~/.ssh/authorized_keys` file.

4. Modify the `hosts.config` file in `federated-xgboost/sample/` to reflect the IPs of the parties. Set the first IP to be the IP of the aggregator.

5. Place the training and test data .csv files at each party **at the same location**. We provide sample data in the `federated-xgboost/data/` directory. Replace the paths passed into `fxgb.load_training_data()` and `fxgb.load_test_data()` with your respective paths. These can be absolute or relative paths.
...
* The `--sync-dst-dir` option in the `dmlc-submit command` copies everything in the passed in directory to all worker machines. This means that the training script can initially only be on the aggregator machine, and will be automatically copied over to all parties once the job is submitted.
