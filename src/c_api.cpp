...
extern "C" size_t cipher_iv_size() {
    return CIPHER_IV_SIZE;
}

extern "C" size_t rsa_mod_size() {
    return RSA_MOD_SIZE;
}

extern "C" int get_format_settings(
...
extern "C" size_t cipher_pk_size() {
    return CIPHER_PK_SIZE;
}

extern "C" void get_public_key(
    uint8_t*  evidence_msg,
    uint8_t*  enclave_pem
) {
    oe_evidence_msg_t* evidence_msg_t = reinterpret_cast<oe_evidence_msg_t*>(evidence_msg);
    memcpy(enclave_pem, evidence_msg_t->public_key, CIPHER_PK_SIZE);
}

extern "C" int attest_evidence(
    unsigned char* enclave_signer_pem,
    ...
    return Context::getInstance().m_attestation->AttestEvidence(
        ...
        evidence_msg_t->evidence,
        evidence_msg_t->public_key,
        evidence_msg_t->nonce,
        enclave_signer_pem_size,
        evidence_msg_t->evidence_size,
        CIPHER_PK_SIZE);
}

extern "C" size_t sym_enc_size(
    size_t data_size
) {
    return Context::getInstance().m_crypto->SymEncSize(data_size);
}

extern "C" int sym_enc(
    unsigned char* data,
    size_t data_size,
    uint8_t* sym_key,
    size_t key_size,
...
extern "C" int sign_using_keyfile(
    char *keyfile,
    uint8_t* data,
    size_t data_size,
    uint8_t* signature
...
extern "C" int verify(
    ...
    return Context::getInstance().m_crypto->Verify(
        pem_key,
        data,
        signature,
        data_size
